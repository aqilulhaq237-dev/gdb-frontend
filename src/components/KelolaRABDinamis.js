      ...
      if (response.data.status === 'success') {
        ...
        if (periodeAktif.length > 0) {
          programs = programs.filter(
            prog => prog.status_program !== 'Selesai' && periodeAktif.includes(prog.periode?.toString())
          );
        } else {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'id_biaya') {
      ...
      if (claimValue > 0) {
        ...
        } else if (formData.biaya_maksimal > 0 && claimValue > formData.biaya_maksimal) {
    ...
    <div className="px-3 px-md-4 py-3 w-100">
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <button className="btn btn-outline-warning btn-sm" onClick={() => onNavigate('program-kerja')}>📋 Program Kerja</button>
        ...
        <div className="card-body py-2 px-3">
          <div className="row align-items-end g-2">
            <div className="col-md-8">
              ...
              <select className="form-select form-select-sm" value={selectedProgram} onChange={(e) => setSelectedProgram(e.target.value)}>
                <option value="">-- Pilih Program --</option>
                {programList.map((prog) => (
                  <option key={prog.id_program} value={prog.id_program}>{prog.nama_program} ({prog.periode || '-'}) - {prog.status_program || '?'}</option>
                ))}
              </select>
            </div>
            <div className="col-md-4 text-end">
              <button className="btn btn-success btn-sm w-100" onClick={openAddModal} disabled={!selectedProgram}>➕ Tambah Item RAB</button>
            </div>
          </div>
          {selectedProgram && (
            <small className="text-muted">📂 {getNamaProgram(selectedProgram)} | Periode: {programList.find(p => p.id_program == selectedProgram)?.periode || '-'}</small>
          )}
        </div>
      </div>
      ...
      <div className="card shadow-sm">
        <div className="card-header bg-primary text-white py-2 d-flex justify-content-between align-items-center">
          <h5 className="mb-0 h6">📋 Daftar RAB {selectedProgram ? `- ${getNamaProgram(selectedProgram)}` : ''}</h5>
          <span className="badge bg-light text-dark small">{filteredRAB.length} item</span>
        </div>
        <div className="card-body p-2 p-md-3">
          {!selectedProgram ? (
            <div className="text-center py-4 text-muted small">👆 Pilih program kerja terlebih dahulu untuk melihat RAB</div>
          ) : filteredRAB.length === 0 ? (
            <div className="text-center py-4">
              <span className="fs-3">📭</span>
              <p className="text-muted small mt-2">Belum ada item RAB untuk program ini</p>
              <button className="btn btn-success btn-sm" onClick={openAddModal}>➕ Tambah Item RAB</button>
            </div>
          ) : (
            <>
                    ...
                    return (
                      <tr key={rab.id_rab} className={!isValid ? 'table-warning' : ''}>
                        ...
                        <td className="text-center">
                          {biayaInfo ? (
                            isValid ? <span className="badge bg-success small">✅ Valid</span> : <span className="badge bg-warning text-dark small">⚠️ Tidak Valid</span>
                          ) : <span className="badge bg-secondary small">Manual</span>}
                        </td>
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <button className="btn btn-warning btn-sm" onClick={() => openEditModal(rab)} title="Edit">✏️</button>
                            <button className="btn btn-danger btn-sm" onClick={() => handleDelete(rab.id_rab)} title="Hapus">🗑️</button>
                          </div>
                        </td>
                      </tr>
                    );
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header bg-primary text-white py-2">
                <h5 className="modal-title h6">{editingId ? '✏️ Edit Item RAB' : '➕ Tambah Item RAB'}</h5>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <input type="text" className="form-control form-control-sm bg-light" value={getNamaProgram(selectedProgram)} disabled readOnly />
                  </div>

                  <div className="mb-3">
                    <label className="form-label small fw-bold">Pilih Item Biaya <span className="text-danger">*</span></label>
                    <select className={`form-select form-select-sm ${formErrors.id_biaya ? "border-danger" : ""}`} name="id_biaya" value={formData.id_biaya} onChange={handleChange} required>
                      <option value="">-- Pilih dari Katalog Biaya --</option>
                      {katalogBiaya.map((biaya) => (
                        <option key={biaya.id_biaya} value={biaya.id_biaya}>{biaya.nama_biaya}</option>
                      ))}
                    </select>
                    {formErrors.id_biaya && <small className="text-danger">❌ {formErrors.id_biaya}</small>}
                  </div>
                  ...
                  <div className="mb-3">
                    <label className="form-label small fw-bold">Harga Claim (Rp) <span className="text-danger">*</span></label>
                    <input
                      type="text"
                      ...
                      autoComplete="off"
                    />
                    {claimError ? <small className="text-danger">{claimError}</small> : formErrors.harga_claim ? <small className="text-danger">❌ {formErrors.harga_claim}</small> : <small className="text-muted">✍️ Ketik angka, titik muncul otomatis</small>}
                    {!claimError && formData.harga_claim > 0 && formData.biaya_minimal > 0 && <small className="text-success">✅ Harga claim dalam rentang yang diizinkan</small>}
                  </div>

                  <div className="mb-3">
                    <label className="form-label small fw-bold">Keterangan</label>
                    <textarea className="form-control form-control-sm" name="keterangan" rows="2" value={formData.keterangan} onChange={handleChange} placeholder="Keterangan tambahan (opsional)" />
                  </div>
                </div>
                <div className="modal-footer py-2">
                  <button type="button" className="btn btn-secondary btn-sm" onClick={() => setShowModal(false)}>❌ Batal</button>
                  <button type="submit" className="btn btn-primary btn-sm" disabled={!!claimError}>{editingId ? '💾 Simpan Perubahan' : '💾 Tambah Item'}</button>
                </div>
              </form>
            </div>
          </div>
