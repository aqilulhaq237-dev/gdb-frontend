...
function KelolaRABDinamis({ user, onLogout, onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.harga_claim < formData.biaya_minimal) {
      alert(
        `⚠️ Harga claim tidak boleh kurang dari Rp ${formatRupiah(formData.biaya_minimal)}`,
      );
      return;
    }
    ...
    if (formData.harga_claim > formData.biaya_maksimal) {
      alert(
        `⚠️ Harga claim tidak boleh lebih dari Rp ${formatRupiah(formData.biaya_maksimal)}`,
      );
      return;
    }
